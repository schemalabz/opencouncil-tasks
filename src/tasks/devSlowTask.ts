...
export const devSlowTask: Task<DevSlowTaskArgs, DevSlowTaskResult> = async (args, onProgress) => {
    // Bounded so a caller cannot hold a worker slot indefinitely.
    const iterations = Math.min(Math.max(args.iterations ?? 30, 1), 300);
    const sleepMs = Math.min(Math.max(args.sleepMs ?? 2000, 100), 10_000);
