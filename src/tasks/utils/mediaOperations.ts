...
type ResolutionPreset = {
    /** 9:16 output canvas this landscape resolution maps to (absent = borrow nearest). */
    social?: { width: number; height: number };
};

const RESOLUTION_PRESETS: Record<string, ResolutionPreset> = {
    // NOTE: keep 1280x720 first — getPresetConfig falls back to the first key
    // for unknown input resolutions.
    "1280x720": { social: { width: 720, height: 1280 } },
    "1920x1080": { social: { width: 1080, height: 1920 } },
    "640x360": {}, // borrows 1280x720's social canvas
};
