                ...
                await deps.splitAudioDiarization({ file: audioOnly, maxDuration: MAX_TRANSCRIPTION_SEGMENT_DURATION_SECONDS, diarization }, createProgressHandler("segmenting-video"))
                    .then(async (audioSegments) => {
                        ...
                        return deps.transcribe({
                            ...
                            // The canary is allocated per meeting, so it needs a
                            // key that is the same for every segment of this
                            // meeting and the same on a re-run. The segment URLs
                            // are not: they are regenerated by each upload.
                            meetingKey: request.youtubeUrl
