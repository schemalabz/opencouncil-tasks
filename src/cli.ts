...
program
    ...
    .option('-l, --language <language>', 'Content language of the meeting (el|fr)', 'el')
    .action(async (youtubeUrl: string, options: { outputFile: string; language: string }) => {
        console.log('Running pipeline, output to', options.outputFile);
        const result = await pipeline(
            { youtubeUrl, cityLanguage: options.language as CityLanguage },
            (stage: string, progressPercent: number) => {
...
program
    ...
    .option('-l, --language <language>', 'Content language of the audio (el|fr)', 'el')
    .action(async (url: string, options: { outputFile: string; language: string }) => {
        const result = await transcribe({ segments: [{ url, start: 0 }], language: options.language as CityLanguage }, (stage: string, progressPercent: number) => {
...
program
    ...
    .option('-l, --language <language>', 'Content language of the audio (el|fr)', 'el')
    .action(async (file: string, options: { outputFile: string; voiceprints?: string; language: string }) => {
        ...
        const result = await transcribe({
            segments,
            language: options.language as CityLanguage
...
program
    ...
    .option('-l, --language <language>', 'Content language of the meeting (el|fr)', 'el')
    .action(async (youtubeUrl: string | undefined, options: { outputFile?: string; skipPreflight?: boolean; language: string }) => {
        ...
        try {
            const result = await pipeline(
                { youtubeUrl: url, cityLanguage: options.language as CityLanguage },
                (stage: string, progressPercent: number) => {
                    process.stdout.write(`\r  [${stage}] ${progressPercent.toFixed(2)}%`);
                }
            );
