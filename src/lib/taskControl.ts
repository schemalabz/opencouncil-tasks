...
/**
 * A cancel landing mid-SDK-call surfaces as APIUserAbortError, not
 * TaskCancelledError, so the aborted signal — not the error type — is the
 * authoritative marker.
 *
 * Callers outside the AsyncLocalStorage scope (TaskManager catches after
 * runWithTaskControl has returned) must pass their control explicitly.
 */
export function isCancellation(error: unknown, control: TaskControl | undefined = getTaskControl()): boolean {
    return error instanceof TaskCancelledError || Boolean(control?.cancel.signal.aborted);
}
