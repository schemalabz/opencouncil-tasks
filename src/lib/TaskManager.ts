...
export class TaskManager {
    ...
    private async executeTask<T, R>(
        ...
        try {
            ...
            const cancelled = isCancellation(error, control);
            ...
            if (cancelled) {
                // Still log the underlying error: a genuine crash can land inside the
                // cancel window, and the payload deliberately carries no error field.
                if (error instanceof TaskCancelledError) {
                    console.log(`Task ${taskId} (${taskType}) cancelled`);
                } else {
                    console.log(`Task ${taskId} (${taskType}) cancelled — underlying error:`, error);
                }
            } else {
     ...
     */
    public promoteTask(taskId: string): 'promoted' | 'queued' | null {
        const control = this.taskControls.get(taskId);
        if (!control) {
            // Queued tasks are listed by GET /tasks but have no control yet, so
            // "not found" would be wrong: they are simply not running.
            return this.taskQueue.some(t => t.taskId === taskId) ? 'queued' : null;
        }
    ...
    private async sendCallback(callbackUrl: string, update: CallbackPayload): Promise<void> {
        ...
        // A terminal callback is the last word on a task: lose it and the task is
        // left looking like it is still running. Progress updates are superseded
        // by the next one, so only the terminal one is worth retrying and keeping.
        const isTerminal = update.status === 'success' || update.status === 'error' || update.status === 'cancelled';
        if (isTerminal) {
