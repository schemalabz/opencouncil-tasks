...
const MAX_BATCH_CANCEL_POLLS = 12;           // ~60s before giving up and streaming instead
...
async function resolvePromotedBatch(
    ...
    // Bounded: a batch that will not leave 'canceling' must not hold the caller in
    // the very wait promotion exists to escape. Giving up re-issues via streaming,
    // which at worst pays for a batch result nobody reads.
    let drained = false;
    for (let i = 0; i < MAX_BATCH_CANCEL_POLLS; i++) {
        const status = await client.messages.batches.retrieve(batchId);
        if (status.processing_status === 'ended') { drained = true; break; }
    ...
    if (!drained) {
        console.warn(`Batch ${batchId}: still cancelling after ${MAX_BATCH_CANCEL_POLLS} polls — re-issuing via streaming`);
...
export async function aiChat<T>({ model, systemPrompt, userPrompt, prefillSystemResponse, continueFromPartial, prependToResponse, documentBase64, parseJson = true, maxTokens: maxTokensParam, tools, outputFormat, cacheSystemPrompt = false, batchFirst = false, label }: AiChatOptions): Promise<ResultWithUsage<T>> {
    ...
    try {
        ...
        // mid-request instead of waiting for the next checkpoint.
        const requestOptions: Anthropic.RequestOptions = control ? { signal: control.cancel.signal } : {};
        ...
        const streamOnce = () => withRetry(async () => {
        ...
        let err = e;
        if (isCancellation(e) && !(e instanceof TaskCancelledError)) {
            // Keep the original as the cause: formatApiError and
            // classifyTransientError both walk the chain.
            err = Object.assign(new TaskCancelledError(`Task cancelled during LLM call`), { cause: e });
        }
