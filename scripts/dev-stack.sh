#!/usr/bin/env bash
#
# Runs an arbitrary command under the local dev stack: MinIO (unless real DO
# Spaces credentials are configured) + an ngrok tunnel for the callback server
# and /dev/files proxy. Used by smoke.sh and by CLI experiments that need
# externally-reachable storage/webhooks.
#
# Usage:
#   ./scripts/dev-stack.sh npm run cli -- diarize <url> -O out.json
#
# Prerequisites:
...
if [[ $# -eq 0 ]]; then
    echo "Usage: $0 <command...>   (runs the command under MinIO + ngrok with PUBLIC_URL/DO_SPACES_* exported)"
    exit 1
fi

if [[ "$USE_REAL_SPACES" == "true" ]]; then
    # Let DO_SPACES_* come from .env
